...
public class RegisterAdminActivity extends AppCompatActivity {
    ...
    private void register() {
        ...
        UserAdmin userAdmin = new UserAdmin(roles, fullname, email, phone, username, hashedPass);

        // Lưu thông tin người dùng vào Realtime Database theo thứ tự cột roles, fullname, email, phone, username, password
        mDatabase.child("users").child(username).child("roles").setValue(userAdmin.getRoles());
        mDatabase.child("users").child(username).child("fullname").setValue(userAdmin.getFullname());
        mDatabase.child("users").child(username).child("email").setValue(userAdmin.getEmail());
        mDatabase.child("users").child(username).child("phone").setValue(userAdmin.getPhone());
        mDatabase.child("users").child(username).child("username").setValue(userAdmin.getUsername());
        mDatabase.child("users").child(username).child("password").setValue(userAdmin.getPassword());
